...
def main():
    ...
    while True:
        for e in pygame.event.get():
            if e.type is QUIT:  # noqa
                return
            elif e.type is KEYDOWN and e.key is K_ESCAPE:  # noqa
                return
            elif e.type is KEYDOWN and e.key is K_SPACE:  # noqa
                sustain = True
            elif e.type is KEYUP and e.key is K_SPACE:  # noqa
                sustain = False
                note_no = None
                for key, b in enumerate(keylist):
                    if b:
                        ...
                        keylist[key] = False
            elif e.type is MOUSEBUTTONDOWN and (e.button == 1  # noqa
                                                or e.button == 3):
                x, y = e.pos
                vel = 128 * y // HEIGHT
                key = x // KEY_WIDTH
            ...
            elif e.type is MOUSEBUTTONUP and (e.button == 1  # noqa
                                              or e.button == 3):
                if not sustain:
                    note_no = None
                    for key, b in enumerate(keylist):
        ...
        for key in range(keys):
            ...
            pygame.draw.rect(screen,
                             (WHITE_COLOR,
                              BLACK_COLOR)[KEY_COLOR[(note_start + key) % 12]],
                             (x + 1, 0, KEY_WIDTH - 2, HEIGHT))
            if keylist[key]:
                pygame.draw.rect(screen, COLOR, (x, 0, KEY_WIDTH, HEIGHT), 3)
        clock.tick(FPS)
        pygame.display.flip()
        notes = []
        for key, b in enumerate(keylist):
